# Golf SDK - AR Glass Scenario Analysis (V3)
...
This scenario describes the **cloud-free hole position estimation** system that combines:
- **Camera-based hole detection** (AR glass)
- **GPS positioning** (phone)
- **ML depth estimation** (monocular depth from camera image)
- **IMU orientation** (direction tracking)

The system calculates hole GPS coordinates without cloud dependency.
...
sequenceDiagram
    ...
    Glasses->>Glasses: Capture camera frame
    Glasses->>Glasses: Detect hole/flag<br/>[MicroML object detection]
    
    alt Hole Detected
        Glasses->>MCU: Hole detection event
        activate MCU
        
        MCU->>PhoneApp: Request GPS position
        activate PhoneApp
        
        PhoneApp->>GPS: Get current GPS coordinates
        activate GPS
        GPS-->>PhoneApp: Lat/Lng/Alt
        deactivate GPS
        
        PhoneApp->>PhoneApp: Store user position
        
        MCU->>Glasses: Request IMU orientation
        Glasses->>Glasses: Read IMU (yaw/pitch/roll)
        Glasses-->>MCU: Orientation data
        
        MCU->>PhoneApp: Forward orientation
        PhoneApp->>PhoneApp: Calculate facing direction
        
        PhoneApp->>HeavyML: Request depth estimation
        activate HeavyML
        
        HeavyML->>HeavyML: ML Depth Model<br/>[Monocular depth estimation]<br/>Input: Camera image<br/>Output: Distance to hole
        
        HeavyML-->>PhoneApp: Depth estimate (meters)
        deactivate HeavyML
        
        PhoneApp->>PhoneApp: Triangulation Calculation:<br/>Hole_Pos = User_Pos +<br/>Depth × Direction_Vector
        
        PhoneApp->>PhoneApp: Refine with multiple samples
        
        PhoneApp->>Storage: Save hole position<br/>(GPS + confidence)
        activate Storage
        
        PhoneApp->>Glasses: Display hole info (AR)
        Glasses->>Golfer: Show distance/direction
        
        deactivate Storage
        deactivate PhoneApp
        deactivate MCU
    else No Hole Detected
        Glasses->>Glasses: Continue scanning
    end
...
| Data Type | Generated | Shared | Saved | Retention |
|-----------|-----------|--------|-------|-----------|
| Camera Frame | ✓ | → Phone | ✗ | Processing only |
| Hole Detection Event | ✓ | → MCU → Phone | ✓ | Session log |
| **User GPS Position** | ✓ (Phone) | → Phone App | ✓ | Session + Cache |
| IMU Orientation | ✓ | → MCU → Phone | ✗ | Processing only |
| **Depth Estimate (ML)** | ✓ | Internal | ✓ | Per-hole cache |
| **Hole GPS Coordinates** | ✓ (calculated) | Internal | ✓ | **Persistent Map** |
| Direction Vector | ✓ | Internal | ✗ | Processing only |
| Confidence Score | ✓ | Internal | ✓ | With hole data |
...
block-beta
columns 1
  block:DepthModel["DEPTH ESTIMATION MODEL"]
    columns 1
    D1["Input: Camera image (320x320x3)"]
    D2["Output: Distance to hole (meters)"]
    D3["Model Type: Monocular Depth Estimation"]
    D4["Training Data: Golf course images with ground truth"]
  end
  block:Factors["Depth Factors"]
    columns 1
    F1["• Flag pole size in frame"]
    F2["• Known object dimensions"]
    F3["• Perspective cues"]
    F4["• Ground plane detection"]
  end
  block:Accuracy["Target Accuracy"]
    columns 1
    A1["• Distance: ±2 meters @ 50m range"]
    A2["• Confidence: >80%"]
    A3["• Inference: <100ms"]
  end
```
...
│                                                                  │
│  Calculate:                                                      │
│  1. Convert User_GPS to ECEF (Earth-Centered Earth-Fixed)       │
│  2. Create direction vector from orientation                    │
│  3. Hole_ECEF = User_ECEF + d × direction_vector                │
│  4. Convert Hole_ECEF back to GPS (lat, lng, alt)               │
│                                                                  │
│  Refinement:                                                     │
...
| Benefit | Description |
|---------|-------------|
| **No Cloud Dependency** | All processing on-device (glasses + phone) |
| **Real-time** | Immediate feedback without network latency |
| **Privacy** | GPS data never leaves the device |
| **Offline Capable** | Works in areas with poor connectivity |
| **Progressive Mapping** | Hole positions improve with more observations |
...
- **Version**: 3.0 (Mermaid Format with Actor Icons)
